...
MLO_uboot_copy_sd()
{
	...
	# wait for ${media} to be present first as SD cards can take time to be populated by udev
	echo "Waiting for device ${media}..."
	while [ ! -b "${media}" ]; do
		sleep 1
	done
	echo "Device ${media} detected!"
...
debian_fs_copy_sd()
{
	...
	if [ -f km-bbb-debian13.0.tar.gz ] ;then
		echo "km-bbb-debian13.0.tar.gz found"
	else
		echo "km-bbb-debian13.0.tar.gz not found and download from kmserver"
		echo -e "${Red}wget http://142.93.218.33/elinux/km-bbb-debian13.0.tar.gz${NC}${Purple}"
		wget http://142.93.218.33/elinux/km-bbb-debian13.0.tar.gz
	fi
	echo -e "${Red}sudo gunzip -k km-bbb-debian13.0.tar.gz  ---- wait ---${NC}"
	#sudo tar -xvf km-bbb-debian13.0.tar.gz
	sudo gunzip -k km-bbb-debian13.0.tar.gz
	echo -e "${Red}sudo tar -xvf km-bbb-debian13.0.tar -C /mnt/rootfs/${NC}"
	sudo tar -xvf km-bbb-debian13.0.tar -C /mnt/rootfs/
	echo -e "${Red}sudo sync ...${NC}"
	sudo sync

    #sudo rm  /mnt/rootfs/opt/backup/uboot/MLO
    #sudo rm  /mnt/rootfs/opt/backup/uboot/u-boot.img
	if [ ! -f /mnt/rootfs/opt/backup ] ;then
		echo "Create /mnt/rootfs/opt/backup/uboot directory"
		sudo mkdir -p /mnt/rootfs/opt/backup/uboot
	fi
	...
	if [ ! -f /mnt/rootfs/boot/uboot ] ;then
		echo "Create /mnt/rootfs/boot/uboot directory"
		sudo mkdir -p /mnt/rootfs/boot/uboot
	fi
	...
	echo -e "${Red}sudo rm km-bbb-debian13.0.tar${NC}"
	sudo rm km-bbb-debian13.0.tar
	unmount_all_drive_partitions
}
...
if [ -z "${media}" ]; then
	echo -e "${BRed}check script parameters!! ${NC}${Red}"
	echo ""
	echo ""
	usage
fi

#unset media_selected
#echo -en "${Green}Media selected is [${media}]. Continue (y/n)? ${NC}"
#read media_selected
#if [ "x${media_selected}" != "xy" ] ; then
#		exit
#fi
#echo ""
